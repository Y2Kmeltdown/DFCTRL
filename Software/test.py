...
def binFileHandler(filePath:str):
    intDataList = []
    n = 8
    with open(filePath, "r") as f:
        ...
        for word in dataList:
            print(word)
            test = [int(word[i:i+n],2) for i in range(0, len(word), n)]
            print(test)
            intDataList.extend([int(word[i:i+n],2) for i in range(0, len(word), n)])
    return intDataList
...
def packetGenerator(data:list[int], packetType:str, read:bool = False, startAddress = None, messageSize = None):
    ...
    if read:
        readHeader = 32 #1
        dataList = [0]*messageSize
        #dataList = ["0"*dataWidth]*messageSize
        #print(dataList)
    else:
        readHeader = 0 #0
        dataList = data
...
packetGenerator("", "activation", True,0, 20)

#instructionMemory = "data/15052025_instruction_data.txt"
#data = binFileHandler(instructionMemory)
